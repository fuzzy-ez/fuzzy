...
import fuzzyInstallCode from '../template/fuzzy-ui.js?raw'
import type { ImportMap } from '../utils/import-map'
import { mergeImportMap } from '../utils/import-map'
import { IS_DEV } from '../utils/constant'
import { atou, utoa } from '../utils/helper'
export type VersionKey = 'vue' | 'fuzzy'
...
export const useStore = (initial: Initial) => {
  ...
  watch(
    ...
    (version) => {
      ...
      state.files[FUZZY_UI_FILE] = file
  ...
  function generateOnuInstallCode(version: string, styleSource?: string) {
    ...
    const style = styleSource
      ...
      : getSkyPack('fuzzy-ui', version, '/dist/style.css')

    return fuzzyInstallCode.replace('#STYLE#', style)
  }
